...
/**
 * configuración Cipriano ui
 *
 * @author Bernardo
 * @since 24/05/2014
 */
class CiprianoUISetup {

	/**
	 * inicializamos la aplicación de cipriano ui
	 */
	public static function initialize( $appName = "cipriano_ui"){
...
//		$_SESSION['LAST_ACTIVITY'] = time();
	...
	/**
	 * Inicializamos cipriano core.
	 */
	private static function initializeCore(){

		CiprianoConfig::getInstance()->setDbName("cose_cipriano");

		CiprianoConfig::getInstance()->initialize();
		CiprianoConfig::getInstance()->initLogger(dirname(__DIR__) . "/conf/log4php.xml");

	}


	/**
	 * Inicializamos cipriano ui + Rasty
	 */
	private static function initializeUI( $appName ){
		...
		Logger::log("Logger cipriano ui configurado!");


		//cargamos los componentes de cipriano ui.
		$rastyLoader = LoadRasty::getInstance();
		$rastyLoader->loadXml(

				dirname(__DIR__) . '/conf/rasty.xml',
				RastyConfig::getInstance()->getAppPath() . "src/main/php/Cipriano/UI/",

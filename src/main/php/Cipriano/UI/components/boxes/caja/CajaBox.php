...
namespace Cipriano\UI\components\boxes\caja;

use Cipriano\UI\service\UIVentaService;
...
class CajaBox extends RastyComponent{
	...
	protected function parseXTemplate(XTemplate $xtpl){
		...
		if( !empty($caja )){
			...
			$xtpl->assign("fecha",  CiprianoUIUtils::formatDateToView($caja->getFecha()) );

			$ventas_online_ctacte = UIServiceFactory::getUIVentaService()->getTotalesCajaVentasOnlineCtaCte($caja);
			$xtpl->assign("ventas_online_ctacte",  CiprianoUIUtils::formatMontoToView( $ventas_online_ctacte ) );
			...
			if( $this->getDetalle() ){
				...
				foreach ($totales as $total) {
					...
					$xtpl->assign("importe",  CiprianoUIUtils::formatMontoToView($importe) );
